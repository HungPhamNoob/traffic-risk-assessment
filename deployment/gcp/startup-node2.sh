#!/bin/bash
# deployment/gcp/startup-node2.sh - Chạy tự động khi VM boot
set -e
...
# Install Kafka tools (optional for debugging)
echo "📥 Installing Kafka CLI tools..."
apt-get update -qq
apt-get install -y -qq jq netcat-openbsd > /dev/null 2>&1 || true
