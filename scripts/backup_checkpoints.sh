#!/bin/bash
# scripts/backup_checkpoints.sh - Chạy qua Airflow DAG hàng ngày 2AM

set -e

GCS_BACKUP_BUCKET="${GCS_BUCKET_BACKUPS:-capstone-team4-backups}"
DATE=$(date +%Y%m%d)
RETENTION_DAYS=7
...
# 2. Sync Spark/Flink checkpoint metadata (không sync toàn bộ state)
...
gsutil ls gs://${GCS_BACKUP_BUCKET}/** | grep -E "[0-9]{8}" | while read path; do
  ...
  if [[ -n "$file_date" ]]; then
    ...
    if [[ $age_days -gt $RETENTION_DAYS ]]; then
      gsutil rm "$path"
      echo "🗑️ Deleted: $path (${age_days} days old)"
    fi
  fi
done
