...
# Chạy từ thư mục gốc project: powershell -ExecutionPolicy Bypass -File .\scripts\setup_gcp.ps1
...
# Grant IAP tunnel access cho user hiện tại
$currentUser = gcloud config get-value account
gcloud projects add-iam-policy-binding $ProjectId `
    --member "user:$currentUser" `
    --role "roles/iap.tunnelResourceAccessor" `
    --quiet 2>$null
Write-Host "✅ Firewall & IAP configured." -ForegroundColor Green

# ==========================================
# 3. CREATE STARTUP SCRIPT
# ==========================================
Write-Host "📜 Generating startup script..." -ForegroundColor Yellow
$startupDir = "deployment/gcp"
...
if (!$saExists) {
    Write-Host "⚠️ SA not found. Creating $ServiceAccount..." -ForegroundColor Yellow
...
# 5. CREATE VM NODE 1
# ==========================================
Write-Host "🖥️ Creating VM: node1-control..." -ForegroundColor Yellow
$vmExists = gcloud compute instances describe node1-control --zone=$Zone --project=$ProjectId 2>$null
if ($vmExists) {
    Write-Host "️ node1-control already exists. Skipping creation." -ForegroundColor Yellow
} else {
...
# 6. VERIFY & NEXT STEPS
# ==========================================
Write-Host "`n🔍 Verifying VM status..." -ForegroundColor Cyan
